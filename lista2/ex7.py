'''7.Faça um programa para uma loja de tintas. O programa deverá
pedir o tamanho em metros quadrados da área a ser pintada. Considere
que a cobertura da tinta é de 1 litro para cada 3 metros quadrados e 
que a tinta é vendida   em  latas  de  18  litros,  que  custam  R$  80,00.
Informe  ao  usuário  a  quantidades  de  latas  de  tinta  a  serem compradas 
e o preço total.Obs. : somente são vendidos um número inteiro de latas.'''

metros = int(input("metros: "))
if metros % 54 == 0: #se vai ser latas fechadas(tem tem que abrir outra para complementar)
    latas = metros / 54
else:
    latas = (metros // 54) + 1 # o total de latas + 1(completar o que faltou na lata anterior)
valor = latas * 80
print("%d latas"%latas)
print("Total: R$%.2f"%valor)